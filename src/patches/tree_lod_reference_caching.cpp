            ...
            for (auto& group : data->treeGroups) {
                for (auto& instance : group->instances) {
                    ...
                    // otherwise try cache then scan all files
                        ...
                        {
                            std::shared_lock lock(g_treeReferenceCache.mutex);
                            auto it = g_treeReferenceCache.map.find(baseId);
                            if (it != g_treeReferenceCache.map.end()) {
                                objectReference = it->second;
                                found = true;
                            }
                        }
                        ...
                        if (!found) {
                            ...
                            // Insert even if it's a null pointer
                            {
                                ...
                                g_treeReferenceCache.map.emplace(baseId, objectReference);
                            }
                        }
#ifdef SKYRIM_AE
                    }
